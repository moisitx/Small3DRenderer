...
public class RectangleRenderer implements IRenderer {
    ...
    public void render(Graphics g, IShape shape, BufferedImage image) {
        ...
        if (vertices.size() != 4) {
            throw new IllegalArgumentException("TriangleRenderer expects a shape with exactly 4 vertices.");
        }

        Vertex unitary = Vertex.unitaryVector(Vertex.normalVector(vertices.get(0), vertices.get(1), vertices.get(3)));

        drawRectanglePixels(vertices.get(0), vertices.get(1), vertices.get(2), vertices.get(3), image, Shader.getShade(unitary, shape.getColor()));
    ...
    public void drawRectanglePixels(Vertex a, Vertex b, Vertex c, Vertex d, BufferedImage image, Color color) {

        Vertex pixel = new Vertex(0,0,0);

        //Bounds of the square outside the trapezoid (Maxmimum and minimum values of the triangle)
        int maxY = (int) Math.max(0, Math.floor(Math.max(Math.max(Math.max(a.getY(), b.getY()), c.getY()), d.getY())));
        int maxX = (int) Math.max(0, Math.floor(Math.max(Math.max(Math.max(a.getX(), b.getX()), c.getX()),d.getX())));
        int minY = (int) Math.min(image.getHeight() - 1, Math.ceil(Math.min(Math.min(Math.min(a.getY(), b.getY()), c.getY()), d.getY())));
        int minX = (int) Math.min(image.getWidth() - 1, Math.ceil(Math.min(Math.min(Math.min(a.getX(), b.getX()), c.getX()), d.getX())));
        ...
        for (int i = minX; i < maxX; i++) {
            for (int j = minY; j < maxY; j++) {
                ...
                double cd = Math.abs(Utilities.getInstance().shoelaceTriangleArea(c, d, pixel));
                double da = Math.abs(Utilities.getInstance().shoelaceTriangleArea(d, a, pixel));;

                //Approach comparing the three areas with the total one
                double totalArea = Math.abs(Utilities.getInstance().shoelaceTrapezoidArea(a, b, c, d));
                double epsilon = 1e-6; // Small tolerance for floating-point errors

                if (Math.abs((ab + bc + cd + da) - totalArea) < epsilon) {
